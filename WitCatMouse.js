...
function resizeElementInParent(element, type, types, aspectRatio) {
	let   让 parentWidth, parentHeight;

	if   如果 (type) {
		let   让 parent
		If (types === 'zoom'   “放大”) {if   如果 (types === 'zoom'   “放大”) {
			parent = element.parent = element. parentelelement . parentelelement；parentElement.parentElement;
		}
		else   其他的 {
		...
		parentWidth = document.body.clientWidth;parentWidth = document.body   身体.clientWidth；
		parentHeight = document.body.clientHeight;parentHeight = document.body   身体.clientHeight；
	}

	let elementWidth = element.offsetWidth;let   让 elementWidth = element.offsetWidth；
	let elementHeight = element.offsetHeight;elementHeight = element.offsetHeight；

	if (types === 'zoom') {   If (types === 'zoom'   “放大”) {
		let widthRatio = parentWidth / elementWidth;let   让 widthRatio = parentWidth / elementWidth；
		let heightRatio = parentHeight / elementHeight;let   让 height = parentHeight / elementHeight；
		let scale = Math.min(widthRatio, heightRatio);let   让 scale =数学。min (widthRatio heightRatio);
		element.style.transform = 'scale(' + scale + ')';Element.style   风格.transform   变换 = 'scale('   的规模(' scale ')'；
	} else {
		if (parentWidth / parentHeight > aspectRatio) {if   如果 (parentWidth / parentHeight >) {
			element.style.height = parentHeight + 'px';element.style   风格.height   高度 = parentHeight 'px'   “像素”；
			element.style.width = parentWidth / (aspectRatio / (parentWidth / parentHeight)) + 'px';element.style   风格.width   宽度 = parentWidth / （aspectRatio / (parentWidth / parentHeight)）“px”;
		} else {
			element.style.width = parentWidth + 'px';element.style   风格.width   宽度 = parentWidth 'px'   “像素”；
			element.style.height = parentHeight / (aspectRatio / (parentWidth / parentHeight)) + 'px';element.style   风格.height   高度 = parentHeight / （aspectRatio / (parentWidth / parentHeight)）“px”;
		}
	}
  }
